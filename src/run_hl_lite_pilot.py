...
class OCLHighResPath(nn.Module):
    """
    OCL-like high-resolution path.

    对应 HL-SAM-Seg 中的 high-resolution detail path，但做成轻量版:
      输入: RGB 512x512
      输出: high feature 128x128
    """
    def __init__(self, out_ch: int = 128):
        ...
        self.down2 = nn.Sequential(
            nn.Conv2d(32, 64, kernel_size=3, stride=2, padding=1),  # 256
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
        )

        self.down4 = nn.Sequential(
            nn.Conv2d(64, out_ch, kernel_size=3, stride=2, padding=1),  # 128
...
class HLCrossFusionLite(nn.Module):
    """
    High-Low Resolution Cross Fusion Lite.

    low:  SAM low-res semantic feature [B, 256, 64, 64]
    high: OCL high-res detail feature [B, 128, 128, 128]

    双向交互:
      1. high -> low: 高分辨率细节池化到 64x64，修正 low semantic feature
      2. low -> high: low semantic feature 上采样到 128x128，给 high detail feature 提供语义 gate
    """
    def __init__(self, low_ch: int = 256, high_ch: int = 128):
        ...
        # 安全初始化：让模型初始接近原 low/high，不会一开始强扰动
...
class HLLiteDecoder(nn.Module):
    ...
    设计:
      SAM low feature 64x64
      OCL high feature 128x128
      HL cross fusion 后，先 low up 到 128x128，与 high 融合，再上采样到 512。
    """
    def __init__(self, low_ch: int = 256, high_ch: int = 128):
        super().__init__()
        ...
        self.low_up128 = nn.Sequential(
            nn.ConvTranspose2d(low_ch, 128, kernel_size=4, stride=2, padding=1),  # 128
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True),
        )
...
class HLLiteAux(nn.Module):
    """
    HL-Lite + boundary auxiliary head.

    第二轮再用。
    如果第一轮 hl_lite 不提升，可以先不要跑这个。
    """
    def __init__(self, low_ch: int = 256, high_ch: int = 128):
        super().__init__()
        self.main = HLLiteDecoder(low_ch=low_ch, high_ch=high_ch)

        self.boundary_head = nn.Sequential(
            nn.Conv2d(high_ch, 64, kernel_size=3, padding=1),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(64, 32, kernel_size=4, stride=2, padding=1),  # 256
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(32, 1, kernel_size=4, stride=2, padding=1),   # 512
        )

    def forward(self, feat: torch.Tensor, rgb: torch.Tensor):
        # 为了兼容旧训练循环，这里默认只返回 mask logits。
        # 如果后续要加 aux boundary loss，再单独改 train loop。
        return self.main(feat, rgb)
...
def build_variant(variant: str) -> nn.Module:
    ...
    if variant == "hl_lite":
        return HLLiteDecoder()

    if variant == "hl_lite_aux":
        return HLLiteAux()
