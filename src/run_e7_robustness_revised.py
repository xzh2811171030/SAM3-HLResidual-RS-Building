...
=============================================================================
功能说明:
  1. 在 5 档噪声标准差 sigma ∈ {0, 2, 5, 10, 20} 下评估 2 个核心模型
  2. 2 个模型:
     - Model 1: E5-FT 目标域 LoRA 微调, box 几何抖动噪声
     - Model 2: E6 GBG-SAM3 完整方案 + UG-DP (d=5), box 几何抖动噪声
  3. 多随机种子 (42/123/456) 求均值作为折线图数据点
  4. 双轨制: --full_test 启用全量 8,402 张测试集
  5. 输出: results/robustness_curve.png + results/robustness_metrics.json

用法:
  python src/run_e7_robustness_revised.py
  python src/run_e7_robustness_revised.py --full_test
  python src/run_e7_robustness_revised.py --sigma 0,2,5,10,20 --seeds 42,123,456
=============================================================================
"""
...
def inject_lora_to_vit(model, rank: int = 8, alpha: int = 16, dropout: float = 0.05) -> None:
    ...
    # 评估模式下，冻结所有非 LoRA 参数，只保留 LoRA 可训练（用于加载权重）
...
# ==========================================================================
# 模块 9: [已废弃] 原 Model 1 --- SAM3 Zero-shot 评估 (像素级高斯噪声)
#    本脚本不再调用, 函数已移至 run_e7_robustness.py
# ==========================================================================
@torch.no_grad()
...
# ==========================================================================
# 模块 10: Model 1 --- E5-FT LoRA 评估 (box 几何抖动噪声)
...
def evaluate_model23_lora_boxnoise(
    ...
    for idx in tqdm(range(len(dataset)),
                    desc=f"  M1 {domain_short}_20shot σ={sigma} s={seed}",
                    unit="img", ncols=100):
        sample = dataset[idx]
        img_tensor = sample["image"]
        mask = sample["mask"].squeeze(0).numpy()
...
# ==========================================================================
# 模块 11: Model 2 --- GBG-SAM3 评估 (Prompt-Free, 无 BBox 输入)
#     [当前被注释, 暂不调用]
#     v2.0 修复: 彻底删除图像平移逻辑。
...
#
#     在 run_robustness_eval 之前执行, 完整扫描所需的权重文件
...
def check_all_weights(
    ...
    for seed in seeds:
        expected.append({
            "model": "M1 E5-FT (Tgt LoRA)",
            "name": f"sam3_lora_tgt_20shot_seed{seed}.pth",
            "mandatory": True,
        })
        expected.append({
            "model": "M2 GBG-SAM3 (Ours)",
    ...
    print(f"  待检文件: {len(expected)} 个  (2 models × {len(seeds)} seeds)")
    print(f"{'='*85}")
    ...
    if missing:
        ...
        if any("tgt" in m and "gbg" not in m.lower() for m in missing):
            print(f"    # 目标域 LoRA 20-shot (M1 依赖)")
            print(f"    python src/run_all_experiments.py --domain target --shots 20 --mode lora")
        if any("gbg" in m.lower() for m in missing):
            print(f"    # GBG-SAM3 目标域 20-shot (M2 依赖)")
...
def run_robustness_eval(
    ...
    for sigma in sigmas:
        ...
        # --- Model 1: E5-FT 目标域微调 (目标域 20-shot LoRA) ---
        print(f"\n  [Model 1] E5-FT 目标域微调 (目标域 20-shot LoRA)")
        m1_seed_results: List[Dict] = []
        for seed in seeds:
            try:
                ...
                m1_seed_results.append({"mIoU": m["mIoU"], "F1": m["F1"],
        ...
        if m1_seed_results:
            miou_mean, miou_std = aggregate_over_seeds(m1_seed_results, "mIoU")
            f1_mean, _ = aggregate_over_seeds(m1_seed_results, "F1")
            biou_mean, _ = aggregate_over_seeds(m1_seed_results, "Boundary_IoU")
            results["E5-FT (Tgt LoRA)"][sigma] = {
                "mIoU": miou_mean, "mIoU_std": miou_std,
                "F1": f1_mean, "Boundary_IoU": biou_mean,
            }
            raw_data["E5-FT (Tgt LoRA)"][sigma] = m1_seed_results
            print(f"    Mean mIoU={miou_mean:.4f} ± {miou_std:.4f}  "
                  f"(seeds={seeds}, {len(m1_seed_results)}/{len(seeds)} 成功)")

        # --- Model 2: GBG-SAM3 (多 seed 均值, UG-DP d=5) ---
        print(f"\n  [Model 2] GBG-SAM3 完整方案 (UG-DP d=5)")
        ...
        if m2_seed_results:
            ...
            results["GBG-SAM3 (Ours)"][sigma] = {
                "mIoU": miou_mean, "mIoU_std": miou_std,
                "F1": f1_mean, "Boundary_IoU": biou_mean,
            }
            raw_data["GBG-SAM3 (Ours)"][sigma] = m2_seed_results
    ...
    output = {
        ...
        "metadata": {
            ...
            "noise_types": {
                ...
                "GBG-SAM3 (Ours)": "Prompt-Free (no BBox), immune to box jitter by design",
            },
        },
...
def main() -> None:
    ...
    if not args.skip_preflight:
        ok = check_all_weights(seeds, WEIGHTS_DIR)
        if not ok:
            sys.exit(1)
