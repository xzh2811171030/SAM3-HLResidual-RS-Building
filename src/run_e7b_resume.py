...
run_e7b_resume.py
=============================================================================
E7b: Image Corruption Robustness (Resume — 只跑未完成的腐蚀)

目的：
  在 run_e7b_image_corruption.py 部分完成后，续跑剩余的图像腐蚀实验。
  仅评估 HL-Residual 模型。

剩余腐蚀：
  brightness_1.3, contrast_0.7, contrast_1.3, downsample_0.5, downsample_0.25,
  occlusion_0.1, occlusion_0.2
...
def parse_corruption_specs(preset: str) -> List[Tuple[str, float]]:
    # 只跑缺失的腐蚀（resume 模式，忽略 preset 参数）
    return [
        ('brightness', 1.3),
        ('contrast', 0.7),
        ('contrast', 1.3),
...
def plot_corruption_bars(results: Dict, out_path: Path):
    ...
    ax.bar(x, ours_vals, width, label="HL-Residual (Ours)")

    ax.set_ylabel("mIoU (%)")
    ax.set_title("Image Corruption Robustness (Resume)")
    ax.set_xticks(x)
...
def parse_args():
    parser = argparse.ArgumentParser("E7b image corruption robustness (resume)")
...
def main():
    ...
    print("=" * 90)
    print("E7b Image Corruption Robustness (Resume)")
    print("=" * 90)
    ...
    summary = {
        "experiment": "E7b_image_corruption_resume",
        "metadata": {
            ...
            "use_tta": args.use_tta,
            "note": "Resume run — HL-Residual only. Validation calibration is performed on clean target_val; corrupted test is evaluated using the clean-val-selected postprocess config.",
        },
        "results": results,
        "raw_per_seed": raw,
    }

    json_path = out_dir / "e7b_image_corruption_resume_metrics.json"
    json_path.write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")

    fig_path = out_dir / "e7b_image_corruption_resume_miou.png"
    plot_corruption_bars(results, fig_path)

    print("\n" + "=" * 90)
    print("E7b resume finished")
    print("=" * 90)
