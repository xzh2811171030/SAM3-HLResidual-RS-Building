...
class HLResidualBranch(nn.Module):
    """
    只学习高分辨率残差，不替代原 lora_light decoder。
    """
...
class HLResidualRefiner(nn.Module):
    ...
    """
    def __init__(self, base_decoder: LightweightMaskDecoder):
        super().__init__()
...
def load_lora_light_checkpoint(
    ...
    if "model" not in ckpt:
        raise KeyError(f"checkpoint 缺少 model: {base_ckpt_path}")

    base_decoder.load_state_dict(ckpt["model"], strict=True)
...
def train_refiner(
    ...
    model = HLResidualRefiner(base_decoder).to(DEVICE)
    ...
    best_val = -1.0
    best_path = out_dir / "weights" / "hl_residual_best.pth"
    best_path.parent.mkdir(parents=True, exist_ok=True)

    history = []

    print("\n" + "=" * 90)
    print("Train HL-Residual Refiner")
    print("=" * 90)
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % val_every == 0 or epoch == epochs:
            if select_by_calibrated_val:
                val_select_info = quick_val_select_score(model, extractor, val_ckpt_loader)
    ...
    result_path = out_dir / "hl_residual_summary.json"
    result_path.write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8")

    print("\n" + "=" * 90)
    print("HL-Residual summary")
    print("=" * 90)
    print(
