"""
=============================================================================
train_fully_supervised_seg.py  ---  全监督基线训练 v3.0 (SegFormer only)
=============================================================================
v3.0 重构:
...
用法:
  python src/training/train_fully_supervised_seg.py
  python src/training/train_fully_supervised_seg.py --full_test
  python src/training/train_fully_supervised_seg.py --seeds 42,123,456
  python src/training/train_fully_supervised_seg.py --epochs 50
=============================================================================
"""
...
def evaluate_on_test_set(
    ...
    for batch in tqdm(test_loader, desc="  测试 SegFormer", unit="batch", ncols=100):
        images = batch["image"].to(DEVICE)
        masks = batch["mask"]
...
def run_fully_supervised_experiment(
    ...
    print(f"\n{'='*70}")
    print(f"  全监督基线训练 v3.0 (SegFormer)")
    ...
    for idx, seed in enumerate(seeds):
        ...
        try:
            ...
            test_metrics["best_val_iou"] = seg_result["best_val_iou"]
            test_metrics["seed"] = seed
            seed_metrics.append(test_metrics)
            print(f"  SegFormer seed={seed}: mIoU={test_metrics['mIoU']:.4f}  "
                  f"F1={test_metrics['F1']:.4f}  "
                  f"BIoU={test_metrics['Boundary_IoU']:.4f}")
...
def print_summary_table(
    ...
    print(f"\n{'='*85}")
    print(f"  SegFormer 全监督基线 学术指标汇总 | 测试集: {test_label} | 种子: {seeds}")
    print(f"{'='*85}")

    mious = [e["mIoU"] for e in seed_metrics]
    f1s = [e["F1"] for e in seed_metrics]
    bious = [e["Boundary_IoU"] for e in seed_metrics]
    ...
    print(f"  | {'SegFormer (mit-b2)':<20} | {miou_str:>20} | "
    ...
    for e, s in zip(seed_metrics, seeds):
...
def save_results_json(
    ...
    _paths = get_domain_paths("source", full_test=full_test)
    results_dir = _paths.get("results_dir", str(Path(_paths["project_root"]) / "results"))
    Path(results_dir).mkdir(parents=True, exist_ok=True)

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    test_label = "full" if full_test else "slim"
    filename = f"segformer_supervised_{test_label}_{timestamp}.json"
    filepath = str(Path(results_dir) / filename)

    output = {
        "experiment": "SegFormer_Fully_Supervised",
        "metadata": {
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="全监督基线训练 v3.0 (SegFormer only)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  python src/training/train_fully_supervised_seg.py
  python src/training/train_fully_supervised_seg.py --full_test
  python src/training/train_fully_supervised_seg.py --seeds 42
  python src/training/train_fully_supervised_seg.py --seeds 42,123,456 --epochs 50 --full_test
        """,
    )
    parser.add_argument(
